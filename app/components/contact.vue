<template>
  <section id="contact" class="mx-auto flex flex-col items-center border-t p-8 text-center">
    <header class="flex flex-col items-center gap-4 text-center">
      <h2 v-motion :initial="{ opacity: 0, y: -20 }" :visible-once="{ opacity: 1, y: 0 }" :duration="1000">
        {{ t("index.contact.title") }}
      </h2>
      <p
        v-motion :initial="{ opacity: 0, y: -20 }"
        :visible-once="{ opacity: 1, y: 0 }" :duration="1000"
        class="text-muted-foreground text-sm leading-5 md:whitespace-nowrap"
      >
        {{ t("index.contact.subtitle") }}
      </p>
    </header>

    <div
      v-motion :initial="{ opacity: 0, y: 20 }"
      :visible="{ opacity: 1, y: 0 }" :duration="1000"
      class="my-12 flex w-full flex-col items-center gap-8"
    >
      <div class="grid gap-8 md:grid-cols-3">
        <nuxt-link
          v-for="link in contactLinks" :key="link.url"
          :to="link.url" target="_blank"
          class="group flex flex-row items-center gap-4"
        >
          <icon :name="link.icon" size="60" class="text-primary group-hover:scale duration-500" />
          <div class="flex flex-col items-start gap-1">
            <span class="font-semibold">{{ link.label }}</span>
            <span class="text-muted-foreground text-sm group-hover:underline">{{ link.text }}</span>
          </div>
        </nuxt-link>
      </div>
    </div>
  </section>
</template>

<script setup lang="ts">
const { t } = useI18n()

const contactLinks = [
  {
    url: "https://github.com/matimortari",
    icon: "lineicons:github",
    label: "GitHub",
    text: "/matimortari",
  },
  {
    url: "https://linkedin.com/in/matheus-mortari-19rt",
    icon: "lineicons:linkedin",
    label: "LinkedIn",
    text: "/matheus-mortari-19rt",
  },
  {
    url: "mailto:matheus.felipe.19rt@gmail.com",
    icon: "material-symbols:mail-rounded",
    label: "Email",
    text: "matheus.felipe.19rt@gmail.com",
  },
]
</script>
